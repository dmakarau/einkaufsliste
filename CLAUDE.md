...
**Sharing model:** A list is shared with a group by setting `family_group_id`. RLS lets all accepted members read and write items in shared lists. Owner controls INSERT/UPDATE/DELETE on the list record itself. Only the list owner sees the share/unshare option in the UI (`ListenScreen._showShareMenu` checks `list.ownerId == currentUserId`). When an admin removes a member, a Postgres trigger (`unshare_lists_on_member_removal`, `SECURITY DEFINER`) automatically sets `family_group_id = null` on all lists owned by that member in the group — this runs server-side so the admin's RLS cannot block it.
