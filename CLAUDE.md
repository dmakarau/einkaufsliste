...
|-------|------------|--------|
| `ShoppingListModel` | 0 | id, name, isDefault, createdAt, familyGroupId (HiveField 4, nullable), ownerId (HiveField 5, nullable) |
...
**Sharing model:** A list is shared with a group by setting `family_group_id`. RLS lets all accepted members read and write items in shared lists. Owner controls INSERT/UPDATE/DELETE on the list record itself. `ListenScreen._showShareMenu` only shows the share/unshare sheet to the list owner (`list.ownerId == currentUserId`); non-owners and null-ownerId lists get a snackbar (`nurEigentuemerKannTeilen`) instead. When a member is removed from the group (by admin or by leaving voluntarily), a Postgres trigger (`unshare_lists_on_member_removal`, `SECURITY DEFINER`) automatically sets `family_group_id = null` on all lists owned by that member in the group — this runs server-side so the admin's RLS cannot block it.

**Realtime:** `shopping_lists`, `shopping_items`, and `family_group_members` are in the Supabase realtime publication. `ShoppingListCubit` subscribes to `shopping_lists` changes (filtered by `family_group_id`) when the user is in a group. Item changes propagate via a Postgres trigger (`shopping_items_touch_list`) that bumps `shopping_lists.updated_at` — the app does not subscribe to `shopping_items` Realtime events directly (Supabase cannot reliably evaluate the complex group-membership RLS policy at Realtime event time). When a list is unshared (`family_group_id → null`), the Realtime UPDATE event is dropped by RLS (member can no longer see the row), so `SupabaseSyncService.unshareList()` also sends a Broadcast message (`event: 'list_unshared'`) on the group channel; `subscribeToGroupChanges()` listens for this broadcast to trigger `syncFromRemote()`. `FamilyCubit` subscribes to `family_group_members` via two channels: `'members_$groupId'` (admin/member list updates — INSERT/UPDATE/DELETE) and `'invites_$uid'` (incoming invite notification for users not yet in a group — INSERT only). For `family_group_members` Realtime to work, three Supabase-side requirements must be met — see `.claude/rules/supabase.md` for the required SQL.

**`FamilyCubit` → `ShoppingListCubit` wiring (`main.dart`):** Two separate `BlocListener`s on `FamilyCubit` are used instead of one. (1) `watchGroup` listener: fires only on `prev is! FamilyHasGroup → FamilyHasGroup` to avoid re-subscribing the Realtime channel on every member list refresh. (2) `syncFromRemote` listener: fires on **every** `FamilyHasGroup` emission — this is necessary because `_refreshMembers()` emits `FamilyHasGroup → FamilyHasGroup` directly (no `FamilyLoading` in between), so a single `prev is! FamilyHasGroup` condition would miss member-leave Realtime events on the admin's device. The `FamilyNoGroup` branch in the second listener calls `stopWatching()` + `syncFromRemote()`, guarded by `AuthAuthenticated` to avoid a stale-JWT call during sign-out.
